...
Sync.prototype.processAddressInfo = function (addrInfo, cb) {
  ...
  var self = this;
  // addrInfo.address = 'UexWApWdGr8UDwaEtZn12R4BGcbfxtQfeZ';
  // addrInfo.balance = 102;
  // if(addrInfo.balance)
  //if (addrInfo.balanceSat > 0) {
  self.txDb.updateRankingEx(addrInfo, function (err) {
    if (err) return console.log('Ooops!', err);
  });
  /*
  self.txDb.putAddressInfo(addrInfo, function (err) {
    if (err) return console.log('Ooops!', err);

    self.txDb.getAddressInfo(addrInfo.address, function (err, info) {
      if (err) return console.log('Ooops!', err);
      console.log(info);
    });
...
Sync.prototype.storeTipBlock = function (b, allowReorgs, cb) {
  ...
  // by hms
  // console.log(">>>HOOK: tx list");
  // console.log(b.tx);

  // console.log("blockhash: %s", b.hash);
  // b.tx.forEach(function (txid) {
        ...
        self.txDb.fromIdWithInfo(txid, function (err, tx) {
          ...
          if (addrList.length > 0) {
            ...
            self._processAddresses(addrList, function (err, addrInfo) {
              if (err) return cb(err);
              // console.log(">>>HOOK: address = %s, balance = %s", addrInfo.address, addrInfo.balance);
              // console.log(addrInfo);
