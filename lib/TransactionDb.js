...
var encodedData = require('soop').load('unifycore/util/EncodedData', {
  base58: base58
});
var versionedData = require('soop').load('unifycore/util/VersionedData', {
  parent: encodedData
});

var Address = require('soop').load('unifycore/lib/Address', {
...
TransactionDb.prototype.updateRankingEx = function (addrInfo, cb) {
  var self = this;
  ...
  db.get(kLast, function (err, val) {
    ...
    self.getRanking(function (ranking, addrs) {
      if (addrs.indexOf(addrInfo.address) < 0) {
        ...
        for (var i = 0; i < RANKING_MAX + 1; i++) {
          if (ranking[i].address == addrInfo.address) {
            ranking[i].balance = addrInfo.balance;
            break;
          }
        }
      }
...
TransactionDb.prototype.updateRanking = function (addrInfo, cb) {
  ...
  db.get(kLast, function (err, val) {
    ...
    var vv = val.split(':');
    var addr = vv[0];
    var bal = parseInt(vv[1]);

    self.getRanking(function (ranking, addrList) {
      // console.log("######################################################################################");
      // console.log(addrInfo);
      // console.log("++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++")
      // console.log(ranking);
      var aaBal = addrInfo.balanceSat;
      var pos = 0;
      for (var i = 98; i > -1; i--) {
        var kk = RANK_PREFIX + i;
        var kkBal = ranking[kk].balance;
        if (aaBal <= kkBal) {
          pos = i + 1;
          break;
        }
      ...
      for (var i = 99; i > pos; i--) {
        var j = i - 1;
        // ranking[RANK_PREFIX + i] = ranking[RANK_PREFIX + j];
        var obj = ranking[RANK_PREFIX + j];
        dbScript.push({
          type: 'put',
          key: RANK_PREFIX + i,
          value: obj.address + ':' + obj.balance
        });
      }
      self._batch(dbScript, cb);
      /*
      ranking[RANK_PREFIX + pos] = {
        ranking: pos,
        address: addrInfo.address,
        balance: aaBal
      };
      ...
      // console.log("####################################################################################################");
      var dbScript = [];
      ranking.forEach(function(rk, rv){
        dbScript.push({
          type: 'put',
          key: rk,
          value: rv.address + ':' + rv.balance
        });
      });
      self._batch(dbScript, cb);
      */
